...
const flowiseWidget =
  document.querySelector('flowise-chatbot') || document.querySelector('flowise-fullchatbot') || document.querySelector('[class*="chatbot"]');
...
if (shadowRoot) {
  ...
  if (mainContainer) {
    ...
    const messagesContainer =
      mainContainer.querySelector('.flex-1.overflow-y-auto') ||
    ...
    if (messagesContainer) {
      ...
      hostMessages.forEach((msg, index) => {
        ...
        // Avatar'ı bul
        const avatar = msg.querySelector('figure[data-testid="default-avatar"]') || msg.querySelector('[class*="avatar"]') || msg.children[0];
        ...
        const bubble = msg.querySelector('[data-testid="host-bubble"]') || msg.querySelector('.prose') || msg.querySelector('span');
...
if (shadowRoot) {
  ...
  const leadForm = shadowRoot.querySelector('.lead-capture-container') || shadowRoot.querySelector('[data-testid="host-bubble"]');
  if (leadForm) {
...
if (flowiseWidget) {
  ...
  if (shadowRoot) {
    const botContainer =
      shadowRoot.querySelector('[part="bot"]') ||
...
function analyzeElementOverflow(element, name) {
  ...
  if (parentRect) {
    ...
    analysis.overflow.contentOverflowsContainer = analysis.parentRelation.exceedsRight || analysis.parentRelation.exceedsBottom;
  }
...
if (shadowRoot) {
  // Widget container
  const widgetContainer = shadowRoot.querySelector('.flowise-widget-container') || shadowRoot.querySelector('[style*="position"]');
  if (widgetContainer) {
  ...
  hostMessages.forEach((msg, index) => {
    ...
    // Message content
    const messageContent = msg.querySelector('[data-testid="host-bubble"]') || msg.querySelector('[class*="bubble"]') || msg.children[1];
    if (messageContent) {
